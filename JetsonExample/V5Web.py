...
class V5WebData:
    ...
    def __init__(self, v5Map, v5Pos, image_processor, port = None):
    ...
    def __getColorCorrection(self):
        # Returns the current color correction values
    ...
    def __getColorElement(self):
        ...
        if(len(pixelData) > 0):
            ...
            buffer = cv2.imencode(".jpeg", cv2.cvtColor(pixelData, cv2.COLOR_BGR2RGB))[1]
    ...
    def __message_received(self, client, server, message):
        ...
        if (message.startswith("set_gps_offset")):
        ...
        elif (message.startswith("set_color_correction")):
            new_values = message.split(',')[1:]
            new_values = [int(value) for value in new_values]
            new_color_correction = ColorCorrection(*new_values)
            self.setColorCorrection(new_color_correction)
        else:
            if len(message) > 200:
                message = message[:200]+'..'
            ...
            for cmd in cmds:
                ...
                elif(cmd == "get_color_correction"):
                    outData["ColorCorrection"] = self.__getColorCorrection().__dict__
    ...
    def setColorCorrection(self, colorCorrection):
        # Updates the color correction and saves it to a JSON file
        self.__dataLock.acquire()
        self.__colorCorrection = colorCorrection
        if (self.__image_processor is not None):
            self.__image_processor.updateHSV(colorCorrection)
        self.__colorCorrection.to_JSON("color_correction.json")
        self.__dataLock.release()
...
class ColorCorrection:
    ...
    @classmethod
    def from_JSON(cls, file_name):
        # Loads in the ColorCorrection data from a JSON file, creates one with default values if it does not yet exist in the current directory
        try:
            # Get the directory of the current script
            script_dir = os.path.dirname(os.path.realpath(__file__))
            ...
            with open(file_path, 'r') as f:
                data = json.load(f)
            return cls(data['h'], data['s'], data['v'])
        except (FileNotFoundError, KeyError, ValueError):
            new_instance = cls(0, 0, 0)
            new_instance.to_JSON(file_name)
            return new_instance

    def to_JSON(self, file_name):
        # Writes the current ColorCorrection data to a JSON file, updating the file so the local offset is always current with the stored offset
        # Get the directory of the current script
        script_dir = os.path.dirname(os.path.realpath(__file__))

        # Combine the script's directory with the file name
        file_path = os.path.join(script_dir, file_name)
        data = {'h': self.h, 's': self.s, 'v': self.v}
        with open(file_path, 'w') as f:
            json.dump(data, f)